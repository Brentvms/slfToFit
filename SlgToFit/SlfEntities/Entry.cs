...
namespace SlfToFit.SlfEntities
{
		...
		int speedTime)
	{
		...
		public readonly byte Cadence = (byte)Math.Round(cadence);								// Cadence
		...
		public readonly byte Heartrate = (byte)Math.Round(heartrate);							// Heart rate
		...
		public readonly ushort Power = (ushort)Math.Round(power);								// Power
		...
		public readonly sbyte Temperature = (sbyte)Math.Round(temperature);						// Temperature
		...
		public readonly byte RightBalance = (byte)Math.Round(rightBalance);						// Right balance
		public readonly byte LeftBalance = (byte)Math.Round(leftBalance);						// Left balance
